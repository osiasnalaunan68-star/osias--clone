  ...
  return (
    <>
      <button
        ...
        className="fixed top-3 right-3 z-40 h-10 w-10 overflow-hidden rounded-full border-2 border-white bg-navy-900 text-sm font-semibold text-white shadow-lg dark:border-slate-700"
      >
        {profile?.photoURL ? (
          ...
          >
            ...
            {!loading && !user && (
              <div className="space-y-4 py-4 text-center">
                ...
                <button onClick={() => setOpen(false)} className="text-sm text-slate-400">Cancel</button>
              </div>
            )}
            ...
            {!loading && user && !deviceLimitReached && !needsFullName && (
              <div className="space-y-4 py-2">
                ...
                <button onClick={() => setOpen(false)} className="w-full text-sm text-slate-400">Close</button>
              </div>
            )}
          </div>
        </div>
